...
def run(args: Namespace):
    ...
    x_1 = np.load("Datasets/rel_npys_1020/RXs.npy")
    x_1_df = x_1
    y_1_df = np.load("Datasets/rel_npys_1020/Rls.npy")
    # 仅保留数值列 → NumPy（尽量不拷贝）
    ...
    if args.normalize_xs:
        x_res = task.denormalize_x(x_res_60)
    if task.is_discrete:
