...
import torch
import torch.nn.functional as F_cal
...
# def supcon_from_lists(z1, z2, y_list, top_frac: float, temperature: float):
#     """
#     z1, z2: [B*m, D] 两个视图的归一化嵌入
#     y_list: [B, m]    每个 list 的目标值
#     返回:   标量 supcon loss
#     """
#     B, m = y_list.shape
#     N = B * m
#
#     # 每个 list 内取 top-k 为“好样本”
#     k = max(1, int(round(m * top_frac)))
#     topk_idx = torch.topk(y_list, k=k, dim=1).indices  # [B, k]
#     good_mask = torch.zeros((B, m), dtype=torch.bool, device=y_list.device)
#     good_mask.scatter_(1, topk_idx, True)              # [B, m]
#     good_flat = good_mask.reshape(-1)                  # [B*m]
#
#     # 组合两视图：2N x D
#     z = torch.cat([z1, z2], dim=0)                    # [2N, D]
#     z = torch.nn.functional.normalize(z, dim=1)
#     lbl = torch.cat([good_flat, good_flat], dim=0)    # [2N]
#     # 每个样本的 list 编号，保证只在同 list 内做对比
#     gid = torch.arange(B, device=z.device).repeat_interleave(m)
#     gid = torch.cat([gid, gid], dim=0)                # [2N]
#
#     # 余弦相似 / 温度
#     sim = (z @ z.t()) / max(1e-6, temperature)        # [2N, 2N]
#     # 去掉自身参与
#     mask_self = torch.eye(sim.size(0), device=sim.device, dtype=torch.bool)
#     sim = sim.masked_fill(mask_self, float('-inf'))
#
#     # 同 list 掩码
#     same_group = gid[:, None] == gid[None, :]         # [2N, 2N]
#     # 正样本：同 list 且 both good
#     pos_mask = same_group & lbl[:, None] & lbl[None, :] & (~mask_self)
#     # 分母：同 list 且非自身
#     den_mask = same_group & (~mask_self)
#
#     # 对每个锚 i，仅在 i 是 good 的情况下计算
#     anchor_mask = lbl                                  # [2N]
#     if anchor_mask.sum() == 0:
#         return sim.new_tensor(0.0)
#
#     # 归一化分母
#     log_denom = torch.logsumexp(sim.masked_fill(~den_mask, float('-inf')), dim=1)  # [2N]
#
#     # 取出 (i,j) 属于正样本的条目，做 -log prob
#     # 对每个 i，平均它的所有正样本
#     loss_terms = []
#     for i in torch.nonzero(anchor_mask, as_tuple=True)[0]:
#         pos_i = pos_mask[i]  # [2N]
#         if pos_i.any():
#             log_prob_pos = sim[i][pos_i] - log_denom[i]  # [#pos_i]
#             loss_terms.append(-log_prob_pos.mean())
#     if len(loss_terms) == 0:
#         return sim.new_tensor(0.0)
#     a = torch.stack(loss_terms).mean()
#     return torch.stack(loss_terms).mean()

def supcon_from_lists(z1, z2, y_list, top_frac: float, temperature: float):
    """
    内存友好版：按 list 逐组计算 SupCon（不再构造全局 [2Bm, 2Bm] 矩阵）
    z1, z2: [B*m, D] 两个视图的嵌入（无需事先归一化）
    y_list: [B, m]   每个 list 的目标（越大越好）
    返回：标量 supcon loss
    """
    B, m = y_list.shape
    ...
    # 预计算各组 top-k 索引
    k = max(1, int(round(m * top_frac)))
    topk_idx_all = torch.topk(y_list, k=k, dim=1).indices  # [B, k]
    ...
    for b in range(B):
        # —— 取第 b 组 —— #
        s, e = b * m, (b + 1) * m
        zb = torch.cat([z1[s:e], z2[s:e]], dim=0)  # [2m, D]
        yb = y_list[b]                              # [m]

        # 该组的“好样本”布尔标签
        good_mask = torch.zeros(m, dtype=torch.bool, device=yb.device)
        good_mask.scatter_(0, topk_idx_all[b], True)                # [m]
        lbl = torch.cat([good_mask, good_mask], dim=0)              # [2m]

        anchors = torch.nonzero(lbl, as_tuple=True)[0]              # 只对好样本当锚点
        if anchors.numel() == 0:
            continue

        # 组内相似度与掩码（只在本组里对比）
        sim = (zb @ zb.t()) / max(1e-6, temperature)                # [2m, 2m]
        eye = torch.eye(2 * m, device=sim.device, dtype=torch.bool)
        sim = sim.masked_fill(eye, float("-inf"))

        # 正样本：同组且双方都是好样本，且去自对角
        pos_mask = (lbl[:, None] & lbl[None, :] & (~eye))           # [2m, 2m]
        # 分母：同组且非自身
        den_mask = ~eye

        # log 分母
        log_denom = torch.logsumexp(sim.masked_fill(~den_mask, float("-inf")), dim=1)  # [2m]

        # 对每个锚 i，平均其所有正样本的 -log prob
        group_terms = []
        for i in anchors:
            pos_i = pos_mask[i]
            if pos_i.any():
                log_prob_pos = sim[i][pos_i] - log_denom[i]         # [#pos_i]
                group_terms.append(-log_prob_pos.mean())

        if group_terms:
            total_loss += torch.stack(group_terms).mean()
            valid_groups += 1
...
class MultKAN(nn.Module):
    ...
    def fit(self, train_loader,validate_loader,iid_loader, x_elites, y_elites, args, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=False, grid_update_num=10, loss_fn=None, lr=0.0005,start_grid_update_step=-1, stop_grid_update_step=50, batch=-1,
              metrics=None, save_fig=False, in_vars=None, out_vars=None, beta=3, save_fig_freq=1, img_folder='./video', singularity_avoiding=False, y_th=1000., reg_metric='edge_forward_spline_n', display_metrics=None, device='cuda',temperature = 0.1):
        '''
        training
        ...
        min_loss = float("inf")
        loss_fn2 =  get_loss_fn(args.loss, **args.loss_config)
        for _ in pbar:
            statistics = {}
            total_loss = 0
            ...
            for i, (x_batch, y_batch) in enumerate(train_loader):
                ...
                if x_batch.dim() > 2:  # e.g. [B, 1000, 60]
                    B, N, F = x_batch.shape
                    x_flat = x_batch.view(B * N, F)  # -> [B*N, F]
                else:
                    x_flat = x_batch  # -> [B, F]

                # ---- 处理 y ----
                if y_batch.dim() > 2:  # e.g. [B, 1000, 1] 或 [B, 1000, K]
                    By, Ny, *rest = y_batch.shape
                    y_flat = y_batch.view(By * Ny, -1)  # -> [B*N, out_dim]
                elif y_batch.dim() == 2:  # e.g. [B, 1] 或 [B, K]
                    y_flat = y_batch
                else:  # [B]
                    y_flat = y_batch.view(-1, 1)
                if _ % grid_update_freq == 0 and _ < stop_grid_update_step and update_grid and _ >= start_grid_update_step:
                    #self.update_grid(dataset['train_input'][train_id])
                    x_for_grid = x_batch
                    if x_for_grid.dim() > 2:
                        x_for_grid = x_for_grid.view(-1, x_for_grid.size(-1))
                    self.update_grid(x_for_grid)
                ...
                if opt == "adam":
                    #pred = self.forward(dataset['train_input'][train_id], singularity_avoiding=singularity_avoiding, y_th=y_th)
                    y_pred = self.forward(x_flat, singularity_avoiding=singularity_avoiding,
                                        y_th=y_th)
                    #train_loss = loss_fn(pred, dataset['train_label'][train_id])
                    y_pred = y_pred.view(By, Ny)
                    y_flat = y_flat.view(By, Ny) #用于非mse
                    loss = loss_fn2(y_pred, y_flat)

                    # 提取数据特征
                    feats_view1 = self.acts[-2]  # [B*m, D]
                    noise = 0.01 * torch.randn_like(x_flat)
                    x_flat_noisy = x_flat + noise
                    temp = self.forward(x_flat_noisy, singularity_avoiding=singularity_avoiding, y_th=y_th)
                    feats_view2 = self.acts[-2]  # [B*m, D]

                    # SupCon：同一 list 内的 top-q 作为“好样本”，两视图联合
                    contrastive_weight = getattr(args, "contrastive_weight", 0.1)
                    top_frac = getattr(args, "contrastive_top_frac", 0.2)
                    loss_ctr = supcon_from_lists(
                        torch.nn.functional.normalize(feats_view1, dim=1),
                        torch.nn.functional.normalize(feats_view2, dim=1),
                        y_flat, top_frac=top_frac, temperature=temperature
                    )
                    loss = loss + loss_ctr * contrastive_weight
                    if self.save_act:
                        if reg_metric == 'edge_backward':
                            self.attribute()
                        if reg_metric == 'node_backward':
            ...
            with torch.no_grad():
                for i, (x_batch, y_batch) in enumerate(validate_loader):
                    ...
                    # ---- 前向 ----
                    y_pred = self.forward(x_flat)

                    # 保证两边都是 [?, 1]
                    y_pred = y_pred.view(y_pred.size(0), -1)
                    y_flat = y_flat.view(y_flat.size(0), -1)
                    y_pred = y_pred.view(By, Ny)  # 还原成 (B, m)
                    y_flat = y_flat.view(By, Ny)  #用于非mse
                    loss = loss_fn2(y_pred, y_flat)
                ...
                for i, (x_batch, y_batch) in enumerate(iid_loader):
                    ...
                    # ---- y 展平 ----
                    if y_batch.dim() > 2:  # [B, N, 1] / [B, N, K]
                        By, Ny, *_ = y_batch.shape
                        y_flat = y_batch.view(By * Ny, -1)  # [B*N, 1]
                    else:
                        y_flat = y_batch.view(y_batch.size(0), -1)  # [B, 1]

                    # ---- 前向 ----
                    y_pred = self.forward(x_flat)
                    y_pred = y_pred.view(y_pred.size(0), -1)  # [B*N, 1]

                    # ---- 直接算 MSE（展平形状一致了）----
                    total_mse += F_cal.mse_loss(
                        input=y_pred, target=y_flat, reduction="mean"
                    ).item()

                    total_rank_corr += spearman_corr(y_pred, y_flat).item()

                    if _ % 50 == 0 or _ == args.n_epochs - 1:
                        y_all = torch.cat((y_all, y_batch), dim=0)
                        predcition_all = torch.cat((predcition_all, y_pred), dim=0)
                ...
                if args.eval_elites:
                    y_pred = self.forward(x_elites)
                    prediction = loss_fn2.score(y_pred)
                    elite_mse = F_cal.mse_loss(
